...
// [[Rcpp::export]]
List mdmtspv_ga2(arma::mat xy, arma::mat dmat, int nSalesmen, int CostType, int popSize, int numIter, double Epsilon=1e-10, bool return_all=false) {
    ...
    for(int p = 0; p < popSize; ++p){
      ...
      for(int sa = 0; sa < nSalesmen; ++sa){
        ...
        if(r0 <= r1){
          ...
          arma::irowvec sap = arma::irowvec(1).fill(rng_range(0));
          arma::irowvec Tour = join_rows(rng_range, sap);  // returns to start
          d(sa) = CalcTourLength2(Tour, dmat);
